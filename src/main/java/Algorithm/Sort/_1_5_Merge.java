...
// Quick과 차이점
// -> Quick: 피벗을 통해 재배치 & 영역을 분할하며 원소를 재배치(Partition)
// -> Merge: 영역을 쪼갤 수 있을 만큼 쪼갬(Merge Sort, 단순히 반으로 나누며) -> Merge 과정에서 정렬이 이뤄짐
// 장점: 순차적인 비교로 정렬을 진행하므로 LinkedList의 정렬이 필요할 때 사용하면 효율적, 안정정렬, 최악의 경우에도 O(n log n) 보장
// -> Merge Sort 는 연결 리스트에서도 분할과 병합을 포인터 연결만으로 수행할 수 있어 매우 효율적
...
public class _1_5_Merge {
  ...
  // 매 merge 마다 두 배열 생성(메모리 할당) & GC 제거가 반복돼 무거움
